...
def run_script():
    ...
    for season in seasons_to_update:
        ...
        for player_list in all_players:
            ...
            if player_list[index_team_abr] is not None and player_list[index_name] not in stats_dict[season][player_list[index_team_abr]].keys():
                ...
                try:
                    ...
                    # lineups_params["TeamID"] = player_list[index_team_id]
                    # lineups_params["Season"] = season

                    # try:
                    #     lineups_response = requests.get(
                    #         'https://stats.nba.com/stats/leaguedashlineups', params=lineups_params, headers=lineups_headers, proxies={
                    #             'http': proxy,
                    #             'https': proxy
                    #         })
                    # except (requests.exceptions.SSLError, requests.exceptions.ProxyError) as e:
        ...
        with open(new_json_path, 'w') as file:
            ...
            print('written')
        check_all_players_have_images(seasons=season)
        with open(new_json_path, 'w') as file:
            json.dump(stats_dict, file)
...
def check_all_players_have_images(seasons):
    for season in seasons:
        season_data = db.reference(f"/all_stats/{season}").get()
        for team in season_data.keys():
            season_team_data = db.reference(
                f"/all_stats/{season}/{team}").get()
            if isinstance(season_team_data, dict):
                for player, p_dict in season_team_data.items():
                    if "img" not in p_dict.keys():
                        print(p_dict["stats"]["PLAYER_ID"])
                        img = get_player_img(
                            p_dict["stats"]["PLAYER_ID"], player)
                        print(img)
                        attr_ref = db.reference(
                            f"/all_stats/{season}/{team}/{player}/img")
                        attr_ref.set(img)


run_script()
check_all_players_have_images(seasons=seasons)
